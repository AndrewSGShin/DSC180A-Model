...
def feature_engineering(data, streaming=None, download_threshold=1000000):
    ...
    if streaming is not None:
        return [percent, time_std, height_std, size_std, size_mean, int(streaming)]
    else:
        return [percent, time_std, height_std, size_std, size_mean]
...
def build_features(datasets, is_streaming=None, upload_threshold=100000, download_threshold=1000000, height=0.5):
    if is_streaming is not None:
        cols = ['is_peak%', 'peak_interval_std', 'peak_height_std', 'pckt_size_std', 'pckt_size_mean', 'is_streaming']
        ...
        cols = ['is_peak%', 'peak_interval_std', 'peak_height_std', 'pckt_size_std', 'pckt_size_mean']
        if type(datasets) == str:
            temp = etl_dataset(csv=datasets, upload_threshold=upload_threshold, download_threshold=download_threshold, height=height)
...
def run_model(test, is_streaming=None):
    train = []
    for i in range(15):
        train.append(build_features('training data/sgs008-amazonprime-1x-vpn-windows-clean-20201102-' + str(i) + '.csv', True))
        train.append(build_features('training data/sgs008-novideo-vpn-windows-20201117-' + str(i) + '.csv', False))
